...
<table class="table" style=" width: 80%; height:500px; margin: 0 auto; margin-top: 10px;  ">
	<tr style="background-color: lightgray;">
		<th style="text-align: left"><%=board.getiBoardNo() %></th>
		<th><%=board.getiBoardTitle()%></th>
		<th><%=board.getCounselorId() %></th>
		<th><%=board.getiBoardDate() %></th>
		<th><%=board.getiBoardViews() %></th>
	</tr>
	<tr>
...
<%-- 		<textarea cols="110px;" rows="15px;" disabled="disabled" style="resize: none; background-color: white;"><%=board.getiBoardContent() %></textarea> --%>
		<div style=" height:500px; background-color: white; "><%=board.getiBoardContent() %></div>
		</td>
	</tr>	
	<tr>
		<td>첨부파일 </td>
...
<hr>
<br>
<button class="btn btn-md b-btn" style="float: left; background-color: #4ea1d3; color: white; "onclick="location.href='/inquiry/list';">목록</button>
<%-- <a href="/inquiry/modify?iboardno=<%=board.getiBoardNo()%>"> --%>
<c:if test="${board.counselorId eq counselorid }">
...
<%-- 	<button class="btn btn-md b-btn" style="float: right; background-color: #4ea1d3; color: white;"id="btnDelete1" onclick="location.href='/inquiry/delete?iboardno=<%=board.getiBoardNo()%>'">삭제</button> --%>
	<button class="btn btn-md b-btn" style="float: right; margin-right: 5px; background-color: #4ea1d3; color: white;" id="btnUpdate" onclick="location.href='/inquiry/modify?iboardno=<%=board.getiBoardNo()%>'">수정</button>
</c:if>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <input type="submit" id="btnDelete1" class="btn btn-danger" data-dismiss="modal" value="확인" onclick="location.href='/inquiry/delete?iboardno=<%=board.getiBoardNo()%>'">
      </div>
